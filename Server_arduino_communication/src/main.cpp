...
void setup()
{
  ...
  // Placing first data in buffer
  sprintf(sendingBuf[indexBuf_empty], "%u:%u", from1, liters1);
...
void loop()
{
  // When an acknowledgement has arrived, increment the index, and empty the buffer at the previous index.
  if (stringComplete)
  {
  ...
  //Each second if possible send serial data to RPI
  ...
  //Receiving Lora communication, adding data to buffer
